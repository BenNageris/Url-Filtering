...
for (var i = 0; i < arr.length; i++) {
    struct[arr[i].simillarword] = arr[i].ratio;
}

function DOMtoString(document_root) {
    var meta = getImportentMetaTags(document);
    if (document.location.host.startsWith("www.google")){ // don't check google search
        console.log('not checking google search');
        return {};
    }
    //new_check_body()
    if (!document.location.host.endsWith(".org") && (meta["type"] == null || meta["type"] != "article") && (meta["og:type"] == null || meta["og:type"] != "article")) {
        if (meta["keywords"] != null && check_keywords(meta["keywords"]) > 4){
            window.location.href = "<PATH_TO_FORBIDDEN_PAGE>/forbbiden.html";
            //alert('keywords');
        }
        else if (meta["description"] != null && check_description(meta["description"])){
            window.location.href = "<PATH_TO_FORBIDDEN_PAGE>/forbbiden.html";
            //alert('description');
        }
        else if (new_check_body()){
            window.location.href = "<PATH_TO_FORBIDDEN_PAGE>/forbbiden.html";
            //alert('body-in');
        }
    }
...
function check_description(desc) {
    desc = desc.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/\b([A-Z]+)([A-Z])([a-z])/, '$1 $2$3');
    desc = desc.toLowerCase().replace(/([a-z])([0-9])/g, '$1 $2').replace(/([0-9])([a-z])/g, '$1 $2');
    var words = desc.split(/[-\.\s\/:_;~|{}\[\]()"',\?<>=+*&^%$#!`\\Â©â’¸]+/).filter(function (el) { return el.length != 0 });
    var flag = false;
    for (var key in struct) {
        if (flag)
            break;
        for (var i = 0; i < words.length; i++) {
            if (words[i].indexOf(key) >= 0) {
                flag = true;
                break;
            }
        }
    ...
    var score=sentimood.analyze(words.join(" ")).score;
    //alert("description score:"+score)
    return score > 0;
}

function check_domain() {
    var domain = window.location.host ? window.location.host : "";
    domain = domain.replace(/([a-z])([A-Z])/g, '$1 $2').replace(/\b([A-Z]+)([A-Z])([a-z])/, '$1 $2$3');
    domain = domain.toLowerCase().replace(/([a-z])([0-9])/g, '$1 $2').replace(/([0-9])([a-z])/g, '$1 $2');
    var words = domain.split(/[-\.\s\/:_;~|{}\[\]()"',\?<>=+*&^%$#!`\\Â©â’¸]+/).filter(function (el) { return el.length != 0 });
    var flag = false;
    for (var key in struct) {
        if (flag)
            break;
    ...
    if (!flag)
        return false;
    var sentimood = new Sentimood();
    return sentimood.analyze(words.join(" ")).score >= 0;
}
...
function getImportentMetaTags(doc) {
    var names = ["keywords", "description", "subject", "type", "og:type" /*, "topic", "summary", "classification", "category"*/];
    var meta_tags = {};
    var metas = doc.getElementsByTagName('meta');
    for (var i = 0; i < metas.length; i++) {
        if (metas[i].name == null || metas[i].name == "" || metas[i].content == null || metas[i].content == "")
            continue;
        var meta_name = metas[i].name.toLowerCase();
        if (names.includes(meta_name) == true)
            meta_tags[meta_name] = metas[i].content;
    }
    return meta_tags;
}
...
function new_check_body() {
    var documents = [];
    var body = document.body.innerText;
    var sentences = body.split(/[.,!?:;-]|\\s/gm);
    for (var j = 0; j < sentences.length; j++) {
        documents.push(sentences[j].trim());
    }
    var topics = topicise(documents);
    console.log('topics:')
    console.log(topics);
    var relevant = [];
    // check if at least one of articles topics in a category/word from the category we should block 
    // comment: checking words from struct [HashMap of the similar words we have from Word2Vec]
    for (var i = 0; i < topics.length; i++) { 
        var word = topics[i].topic_name;
        if (struct[word] != null || struct[word.substring(0, word.length - 1)] != null || struct[word.substring(0, word.length - 2)] != null || struct[word.substring(0, word.length - 3)] != null || struct[word.substring(0, word.length - 4)] != null) {
            relevant.push(word);
        }
    }
    ...
    var scores = [];
    var sum = 0;
    var pos_scores_count = 0;
    var pos_score_max = 0;

    var neg_scores_count = 0;
    var neg_score_min = 0;

    // checking for each word related to blocked category the sentiment of the sentence it apears in 
    // and calculates count of positive & negative sentences, and min & max sentence scores
    console.log("sentences");
    for (i = 0; i < relevant.length; i++) { 
        for (j = 0; j < sentences.length; j++) {
            word = relevant[i];
            var sentence = sentences[j];
            if (sentence.includes(word) || sentence.includes(word.substring(0, word.length - 1)) || sentence.includes(word.substring(0, word.length - 2)) || sentence.includes(word.substring(0, word.length - 3)) || sentence.includes(word.substring(0, word.length - 4))) {
                console.log(sentence+","+sentimood.analyze(sentence).score);
                var score = sentimood.analyze(sentence).score;
                scores.push(score);
                sum += score;
                if (score > 0)
                    pos_scores_count++;
                if (pos_score_max < score)
                    pos_score_max = score;


                if (score < 0)
                    neg_scores_count++;
                if (neg_score_min > score)
                    neg_score_min = score;
    ...
    if (avg > 1) // article is talking about the category as a good thing -- BLOCK
        return true;
    if (avg < -1) // article is talking about the category as a bad thing -- DONT BLOCK
        return false;
    if (Math.abs(avg) < 0.2) // article is neutral 
        return false;
    if (avg > 0) {
        if (check_domain()) // if domain name contains one of the category words - if yes block
            return true;
        if (pos_score_max > 4) // if document sentence contains a very positive sentence about category
            return true;
        if ((parseFloat(pos_scores_count) / scores.length) > 0.5) // if majority of sentences are talking about category in a good way
            return true;
    }

    if (avg < 0 && neg_score_min > -4) {
        if (neg_score_min >= -3) // if the minimum score is not 'so low' but avg is still minus - block
            return true;
        if ((parseFloat(neg_scores_count) / scores.length) < 0.3) // if less than 30% of senteces are talking about category as a bad manner we should block
            return true;
    }
    return false;
}

function topicise(sentences) { // calculates the page topics
    var result = [];
    ...
    for (var k = 0; k < phi.length; k++) {
        ...
        for (var t = 0; t < topTerms; t++) {
            ...
            result.push({ topic_name: topicTerm, ratio: prob / 100.0 });
